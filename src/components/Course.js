...
const Course = ({ droppableId, value, items }) => {
  const { addToast } = useToasts();
    ...
    const {courses, removeCourse, toggleCourseStatus,updateCourse,pending,complete} = useCourseStore(
        (state) => ({
      ...
      {(provided, snapshot) => (
        <div ref={provided.innerRef} {...provided.droppableProps}>
          ...
          <ul style={{ padding: "12px" }}>
            {items.map((course, i) => (
              <Draggable key={course.id} draggableId={course.id.toString()} index={i}>
                {(provided, snapshot) => (
                  <li
                  ...
                  >
                     <span className="course-item-col-1">
                             ...
                             className="delete-btn"><RiDeleteBin5Fill size="24px" /></button>
                              <button 
                             onClick={() => {
                               handleCourseUpdate(course.id,value)
                             }}
                             className="update-btn"><MdOutlineSystemUpdateAlt size="24px"/></button>
                             </div>
                  </li>
